...
import '../../../controller/downloaded_files_controller.dart';
...
class DownloadFiles extends StatefulWidget {
  ...
  const DownloadFiles({Key? key, this.downloadId = ""}) : super(key: key);
...
class _DownloadFilesState extends State<DownloadFiles> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("الملفات التي تم تنزيلها"),
        titleSpacing: -8,
        actions: [
          IconButton(
      ...
      body: GetBuilder<DownloadFilesController>(
          builder: (controller) {
            return FutureBuilder(
              future: FlutterDownloader.loadTasksWithRawQuery(query: "select * from task order by task_id = '${widget.downloadId}' desc, time_created desc;"),
                ...
                if(snapshot.hasData){
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, int i){
                      ...
                      return Material(
                        color: (item.taskId == widget.downloadId)? Colors.blueGrey[200]: Colors.transparent,
                        child: InkWell(
                          onTap: () async {
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      SizedBox(height: 8,),
                                      Text(
                                        "${Jiffy.parseFromMillisecondsSinceEpoch(item.timeCreated).fromNow()}"
                                        + ((item.progress != 100)? " . ${item.progress}%": "")
                                        + " .${(item.status == DownloadTaskStatus.failed)? "فشل" : ""}",
                                        style: TextStyle(
                                          fontFamily: v.fn,
